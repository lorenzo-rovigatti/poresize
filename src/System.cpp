...
void System::init_from_LAMMPS(char *filename) {
	...
	for(int i = 0; i < 3; i++) {
		...
		try {
			box[i] = boost::lexical_cast<double>(boost::trim_copy(split[1])) - boost::lexical_cast<double>(boost::trim_copy(split[0]));
		}
	...
	while(input.good() && !end) {
		if(input.peek() == 't') {
			...
			if(input.good() && line.size() > 0) {
				boost::split(split, line, boost::is_any_of(" "));
				vec3 position = vec3(boost::lexical_cast<double>(split[2]), boost::lexical_cast<double>(split[3]), boost::lexical_cast<double>(split[4]));
...
void System::init_from_oxDNA(char *filename) {
	std::ifstream input(filename);
